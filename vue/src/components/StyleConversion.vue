...
export default {
  ...
  data() {
    return {
      ...
      value: '',
      options: [
        {value: 'Option1', label: '选项一:毕加索',filePath:'/transfers/checkpoints/la_muse.ckpt'},
  ...
  methods: {
    selectedOption(newVal) {
      ...
      if (selectedOption && selectedOption.filePath) {
        // 这里只是模拟设置文件，实际中你可能需要读取文件或其他操作
        this.fileToUpload = { path: selectedOption.filePath, /* 其他文件信息 */ };
      }
    },
    ...
    PassInThePicture() {
      ...
      axios.post('http://127.0.0.1:9300/predict2', formData)
          .then(response => {
            ...
            // 更新识别结果
            // this.predictionImagename = response.data.image;
            // console.log(this.predictionImagename);
